...
const ensureSignature = async (proposal: any, payload: any) => {
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!parsed.success) return badRequest(res, 'Payload inválido', parsed.error.flatten());
    ...
    await supabaseAdmin.from('od_proposal_form_submissions').insert({
      proposal_id: proposal.id,
      payload,
      submitted_at: new Date().toISOString(),
    });

    await supabaseAdmin.from('od_proposals').update({ status: 'form_filled' }).eq('id', proposal.id);

    if (proposal.requires_signature) {
      const { signUrl } = await ensureSignature(proposal, payload);
      return json(res, 200, { next: 'signature', signUrl });
    }

    const { invoiceUrl } = await ensurePayment(proposal, payload);
    return json(res, 200, { next: 'payment', invoiceUrl });
  } catch (err: any) {
    console.error(err);
    return serverError(res, 'Erro ao processar proposta.', err?.message || err);
  }
}
